...
const VansList = () => {
    ...
    useEffect(() => {
      ...
      const vanElements = vans.map((van) => (
            <div key={van.id} className="van-tile rounded  h-[30rem] w-[23rem] lg:h-[35rem] lg:w-[28rem] lg:mb-10 dark:bg-content dark:text-bkg">
                <Link className="w-full h-[15rem]" to={`/vans/${van.id}`}><img className="h-auto w-full rouded rounded-t-md"  src={van.imageUrl} /></Link>
                ...
                <i className={`van-type ${van.type} selected -ml-[16rem] lg:-ml-[20.2rem]`}>{van.type}</i>
            </div>
        ))

    return(
      <div className="van-list-container lg:-mt-10 lg:mb-0 text-center w-full lg:h-[150vh] h-[370vh] pt-10 lg:pt-14 text-content bg-bkg">
